...
# coding: utf-8
"""
SA-QGFL Malaria Detection - Species-Adaptive Training Script

Species-Adaptive Quality-Guided Focal Loss for Multi-Species Plasmodium Detection.
Designed for D3 dataset (5 classes: Uninfected + 4 Plasmodium species).

Three novel components:
1. MCP (Morphological Complexity Priors) - Per-species gamma
2. PWA (Prevalence-Weighted Alpha) - Per-species alpha
3. HQS (Hierarchical Quality Scoring) - Two-tier quality assessment

Usage:
    python cluster_run_saqgfl.py --model yolov8s --seed 123
    python cluster_run_saqgfl.py --model rtdetr --seed 123 --no-hqs
...
parser = argparse.ArgumentParser(
    description='SA-QGFL Training for Multi-Species Plasmodium Detection',
...
# SA-QGFL specific arguments
parser.add_argument('--use-hqs', action='store_true', default=True,
                    help='Enable Hierarchical Quality Scoring (HQS)')
parser.add_argument('--no-hqs', action='store_true',
                    help='Disable Hierarchical Quality Scoring')
parser.add_argument('--difficulty-threshold', type=float, default=0.6,
                    help='SA-QGFL difficulty threshold')
parser.add_argument('--quality-margin', type=float, default=0.3,
                    help='SA-QGFL quality margin')
parser.add_argument('--saqgfl-debug', action='store_true',
                    help='Enable SA-QGFL debug logging')

# MCP Override (advanced - normally use defaults from sa_qgfl_config)
parser.add_argument('--gamma-override', type=str, default=None,
                    help='Override MCP gamma values (JSON format: {"0":2.0,"1":5.0,...})')

# PWA Override (advanced - normally use defaults from sa_qgfl_config)
parser.add_argument('--alpha-override', type=str, default=None,
                    help='Override PWA alpha values (JSON format: {"0":0.01,"1":0.25,...})')
...
parser.add_argument('--seed', type=int, default=42,
                    help='Random seed (use 123, 456, 789 for 3x runs)')
...
print("=" * 70)
print("SA-QGFL MULTI-SPECIES PLASMODIUM DETECTION")
print("Species-Adaptive Quality-Guided Focal Loss")
print("=" * 70)
print(f"Dataset: {args.dataset.upper()} (Multi-Species)")
print(f"Model: {args.model}")
print(f"Task: {args.task}")
print(f"Loss Type: SA-QGFL")
print(f"  - MCP (Morphological Complexity Priors): Enabled")
print(f"  - PWA (Prevalence-Weighted Alpha): Enabled")
print(f"  - HQS (Hierarchical Quality Scoring): {args.use_hqs}")
print(f"  - Difficulty Threshold: {args.difficulty_threshold}")
print(f"  - Quality Margin: {args.quality_margin}")
...
# Species-specific class configuration (same as SA-QGFL config)
ANALYSIS_SPECIES_CLASS_NAMES = {
    0: 'Uninfected',
    1: 'P_falciparum',
    2: 'P_ovale',
    3: 'P_malariae',
...
ANALYSIS_RARE_SPECIES_IDS = [2, 3, 4]  # P_ovale, P_malariae, P_vivax
ANALYSIS_ALL_PARASITE_IDS = [1, 2, 3, 4]  # All species except uninfected

# Confidence thresholds (Guemas methodology)
ANALYSIS_CONF_HIGH = 0.25
ANALYSIS_CONF_LOW = 0.15
...
def analyze_model_decisions_species(model, test_images, model_name, save_dir, conf_threshold=0.25, iou_threshold=0.45, num_samples_to_display=6):
    """
    Analyze model decisions for multi-species Plasmodium detection with SA-QGFL.
    ...
    print(f"Analyzing {len(test_images)} test images for SA-QGFL species detection...")
    print(f"Will display {num_samples_to_display} in output")
    print(f"Saving up to {max_viz_to_save} visualizations")
    print(f"Confidence thresholds: Confident≥{ANALYSIS_CONF_HIGH}, Uncertain=[{ANALYSIS_CONF_LOW},{ANALYSIS_CONF_HIGH})")
    ...
    for img_idx, img_path in enumerate(tqdm(test_images, desc="Decision analysis")):
        ...
        results_normal = model.predict(img_path, conf=ANALYSIS_CONF_HIGH, iou=iou_threshold, agnostic_nms=True, verbose=False)[0]
        results_low = model.predict(img_path, conf=ANALYSIS_CONF_LOW, iou=iou_threshold, agnostic_nms=True, verbose=False)[0]
        ...
        if results_low.boxes is not None:
            for box in results_low.boxes:
                conf = box.conf.item()
                if ANALYSIS_CONF_LOW <= conf < ANALYSIS_CONF_HIGH:
                    low_conf_count += 1
                    cls = int(box.cls.item())
                    low_conf_data.append(f"{ANALYSIS_SPECIES_CLASS_NAMES[cls]}:{conf:.3f}")

        # Calculate metrics per species
        metrics_per_species = {}
        for cls_id in range(5):
            cls_name = ANALYSIS_SPECIES_CLASS_NAMES[cls_id]
        ...
        parasite_tp = sum(metrics_per_species[ANALYSIS_SPECIES_CLASS_NAMES[c]]['tp'] for c in ANALYSIS_ALL_PARASITE_IDS)
        parasite_fp = sum(metrics_per_species[ANALYSIS_SPECIES_CLASS_NAMES[c]]['fp'] for c in ANALYSIS_ALL_PARASITE_IDS)
        parasite_fn = sum(metrics_per_species[ANALYSIS_SPECIES_CLASS_NAMES[c]]['fn'] for c in ANALYSIS_ALL_PARASITE_IDS)
        parasite_gt = sum(gt_per_species[c] for c in ANALYSIS_ALL_PARASITE_IDS)
        ...
        rare_tp = sum(metrics_per_species[ANALYSIS_SPECIES_CLASS_NAMES[c]]['tp'] for c in ANALYSIS_RARE_SPECIES_IDS)
        rare_fn = sum(metrics_per_species[ANALYSIS_SPECIES_CLASS_NAMES[c]]['fn'] for c in ANALYSIS_RARE_SPECIES_IDS)
        rare_gt = sum(gt_per_species[c] for c in ANALYSIS_RARE_SPECIES_IDS)
        ...
        if img_idx < max_viz_to_save:
            ...
            fig.suptitle(f'SA-QGFL Species Decision Analysis - {model_name} - {img_id}', fontsize=16, fontweight='bold')
            ...
            if results_normal.boxes is not None:
                for box in results_normal.boxes:
                    ...
                    color = ANALYSIS_SPECIES_COLORS.get(cls, (128, 128, 128))
            ...
            ax2.set_title(f'SA-QGFL Predictions (Conf≥{ANALYSIS_CONF_HIGH})\n(Total: {pred_total})', fontsize=11)
            ax2.axis('off')
            ...
            if results_low.boxes is not None:
                for box in results_low.boxes:
                    ...
                    if ANALYSIS_CONF_LOW <= conf < ANALYSIS_CONF_HIGH:
                        ...
                        base_color = ANALYSIS_SPECIES_COLORS.get(cls, (128, 128, 128))
            ...
            ax3.imshow(viz_low)
            ax3.set_title(f'Uncertain [{ANALYSIS_CONF_LOW}-{ANALYSIS_CONF_HIGH})\n+{additional_detections} uncertain', fontsize=11)
            ax3.axis('off')
            ...
            if results_low.boxes is not None:
                for box in results_low.boxes:
                    ...
                    if cls in ANALYSIS_ALL_PARASITE_IDS:  # Infected - positive values (RED)
                    ...
                    if ANALYSIS_CONF_LOW < conf < 0.40:
            ...
            if results_low.boxes is not None:
                for box in results_low.boxes:
                    ...
                    if crop.size > 0 and crop.shape[0] > 5 and crop.shape[1] > 5:
                        ...
                        elif cls in ANALYSIS_ALL_PARASITE_IDS:
            ...
            for idx, (category, cells) in enumerate(examples.items()):
                ...
                if cells:
                    ...
                    ax.imshow(montage)
                    species_names = [ANALYSIS_SPECIES_CLASS_NAMES.get(c[2], '?') for c in cells_sorted]
    ...
    print(f"\n{'='*60}")
    print("SA-QGFL SPECIES DECISION ANALYSIS COMPLETE")
    print(f"{'='*60}")
...
def compute_species_recall_variability(model, yolo_path, config, experiment_name, results_dir):
    ...
    for img_path in tqdm(img_files, desc="Analyzing recall variability"):
        ...
        total_parasites = sum(gt_per_species[c] for c in ANALYSIS_ALL_PARASITE_IDS)
        ...
        if results.boxes is not None:
            for box in results.boxes:
                ...
                if pred_class in ANALYSIS_ALL_PARASITE_IDS:
        ...
        rare_gt = sum(gt_per_species[c] for c in ANALYSIS_RARE_SPECIES_IDS)
        rare_tp = sum(species_tp[c] for c in ANALYSIS_RARE_SPECIES_IDS)
    ...
    fig.suptitle('SA-QGFL Species Recall Variability Analysis', fontsize=14, fontweight='bold')
...
def plot_species_stratified_analysis(test_results, experiment_name, save_dir):
    ...
    fig.suptitle('SA-QGFL Stratified Performance Analysis - Species Detection', fontsize=14, fontweight='bold')
...
def plot_per_class_comparison_species(val_results, test_results, config, experiment_name, save_dir):
    ...
    fig.suptitle('SA-QGFL Per-Species Performance: Validation vs Test', fontsize=14, fontweight='bold')
...
# ============================================================================
# SA-QGFL LOSS INTEGRATION
# ============================================================================
print("\n" + "=" * 70)
print("INTEGRATING SA-QGFL LOSS VIA MONKEY-PATCHING")
print("=" * 70)

from src.losses.sa_qgfl_yolo import SAQGFLYOLOLoss
from src.losses.sa_qgfl_rtdetr import SAQGFLRTDETRLoss
from src.losses.sa_qgfl_config import (
...
# Parse alpha/gamma overrides if provided
class_alphas = PWA_ALPHA.copy()
class_gammas = MCP_GAMMA.copy()

if args.alpha_override:
    try:
        override = json.loads(args.alpha_override)
        class_alphas.update({int(k): float(v) for k, v in override.items()})
        print(f"[SA-QGFL] Alpha override applied: {override}")
    except json.JSONDecodeError as e:
        print(f"[WARNING] Could not parse alpha override: {e}")

if args.gamma_override:
    try:
        override = json.loads(args.gamma_override)
        class_gammas.update({int(k): float(v) for k, v in override.items()})
        print(f"[SA-QGFL] Gamma override applied: {override}")
    except json.JSONDecodeError as e:
        print(f"[WARNING] Could not parse gamma override: {e}")

# Print SA-QGFL configuration
print(f"\n[SA-QGFL] Configuration for {NC} classes:")
print(f"  Classes: {list(CLASS_NAMES.values())}")
print(f"\n  PWA Alpha (per-class):")
for c in range(NC):
    print(f"    {CLASS_NAMES[c]}: α = {class_alphas.get(c, 0.5):.3f}")
print(f"\n  MCP Gamma (per-class):")
for c in range(NC):
    print(f"    {CLASS_NAMES[c]}: γ = {class_gammas.get(c, 2.0):.1f}")
if args.use_hqs:
    print(f"\n  HQS Weights:")
    print(f"    Infection detection: {HQS_INFECTION_WEIGHT}")
    print(f"    Species discrimination: {HQS_SPECIES_WEIGHT}")

# Initialize SA-QGFL losses
saqgfl_yolo = SAQGFLYOLOLoss(
    nc=NC,
    ...
    use_hqs=args.use_hqs,
    hqs_infection_weight=HQS_INFECTION_WEIGHT,
    hqs_species_weight=HQS_SPECIES_WEIGHT,
    debug=args.saqgfl_debug
)

saqgfl_rtdetr = SAQGFLRTDETRLoss(
    nc=NC,
    class_alphas=class_alphas,
    class_gammas=class_gammas,
    difficulty_threshold=args.difficulty_threshold,
...
# Store references for monkey-patching
_saqgfl_yolo_loss = saqgfl_yolo
_saqgfl_rtdetr_loss = saqgfl_rtdetr
_saqgfl_args_epochs = args.epochs

# Store original v8DetectionLoss __call__ method
_original_v8loss_call = yolo_loss_module.v8DetectionLoss.__call__

# Monkey-patch YOLO loss
def saqgfl_forward(self, preds, batch):
    """SA-QGFL-enhanced forward pass for YOLO (replaces BCE with SA-QGFL)"""
    ...
    # SA-QGFL Classification loss (REPLACEMENT for BCE)
    loss_items[1] = _saqgfl_yolo_loss(
        pred_scores=pred_scores,
        target_scores=target_scores.to(dtype),
        current_epoch=getattr(self, '_current_epoch', 0),
        total_epochs=_saqgfl_args_epochs
    ) / target_scores_sum

    # Bbox + DFL loss (unchanged)
    if fg_mask.sum():
...
# Apply YOLO monkey-patch
yolo_loss_module.v8DetectionLoss.__call__ = saqgfl_forward
print(f"\n[SA-QGFL] Patched ultralytics.utils.loss.v8DetectionLoss with SA-QGFL")

# ========================================================================
# RT-DETR SA-QGFL PATCHING
# ========================================================================
try:
    ...
    if hasattr(rtdetr_loss_module, 'DETRLoss'):
        ...
        def saqgfl_get_loss_class(self, pred_scores, targets, gt_scores, num_gts, postfix=""):
            """SA-QGFL-enhanced classification loss for RT-DETR"""
            ...
            # SA-QGFL REPLACEMENT for VarifocalLoss
            if num_gts:
                loss_cls = _saqgfl_rtdetr_loss(
                    pred_scores=pred_scores,
                    one_hot=one_hot,
        ...
        rtdetr_loss_module.DETRLoss._get_loss_class = saqgfl_get_loss_class
        print(f"[SA-QGFL] Patched ultralytics.models.utils.loss.DETRLoss._get_loss_class")
        print(f"[SA-QGFL] RT-DETR SA-QGFL integration: ACTIVE")
    else:
        print(f"[SA-QGFL] Warning: DETRLoss not found - RT-DETR patching skipped")

except ImportError as e:
    print(f"[SA-QGFL] Could not import RT-DETR loss module: {e}")
...
if args.use_wandb:
    # SA-QGFL multi-species project name
    ...
    run_name = f"{env_prefix}saqgfl_{config.model_name}_{config.dataset}_s{args.seed:03d}_{timestamp}"
    ...
    wandb_config = {
        ...
        # Per-class SA-QGFL parameters
        'saqgfl/pwa_alpha': class_alphas,
        'saqgfl/mcp_gamma': class_gammas,
        'saqgfl/hqs_infection_weight': HQS_INFECTION_WEIGHT if args.use_hqs else None,
        'saqgfl/hqs_species_weight': HQS_SPECIES_WEIGHT if args.use_hqs else None,
    ...
    wandb.init(
        ...
        name=run_name,
        config=wandb_config,
        tags=['SA-QGFL', 'species', config.model_name, config.dataset, f's{args.seed}']
    )
    ...
    # Log SA-QGFL parameters summary
    print(f"\n[W&B] SA-QGFL Parameters logged:")
    print(f"  - MCP Gamma: {class_gammas}")
    print(f"  - PWA Alpha: {class_alphas}")
    print(f"  - HQS: {args.use_hqs}")

# ============================================================================
# TRAINING
# ============================================================================
print("\n" + "=" * 70)
print("STARTING SA-QGFL TRAINING")
print("=" * 70)

# Results directory
results_base = Path(args.results_dir) / 'saqgfl' / config.dataset / config.task
...
if test_samples:
    decision_df, decision_summary = analyze_model_decisions_species(
        evaluator.model,
        test_samples,
        f"saqgfl_{config.model_name}_s{args.seed:03d}",
        run_dir,
...
plot_species_stratified_analysis(test_results, f"saqgfl_{config.model_name}_s{args.seed:03d}", run_dir)
plot_per_class_comparison_species(val_results, test_results, config, f"saqgfl_{config.model_name}_s{args.seed:03d}", run_dir)

# ============================================================================
# LOG TO WANDB - COMPREHENSIVE LOGGING (Consistent with Binary QGFL)
# ============================================================================
if args.use_wandb:
    import pandas as pd
    ...
    decision_csv_table = run_dir / f"saqgfl_{config.model_name}_s{args.seed:03d}" / 'decision_analysis' / 'decision_analysis_complete.csv'
    ...
    experiment_name = f"saqgfl_{config.model_name}_s{args.seed:03d}"
    ...
    decision_path = run_dir / f"saqgfl_{config.model_name}_s{args.seed:03d}" / 'decision_analysis'
    ...
    experiment_subdir = run_dir / f"saqgfl_{config.model_name}_s{args.seed:03d}"
    custom_viz_files = [
    ...
    if best_model_path.exists():
        model_artifact = wandb.Artifact(
            name=f"{config.dataset}_{config.task}_{config.model_name}_saqgfl_model",
            type="model",
            metadata={
                # Performance metrics
    ...
    data_artifact = wandb.Artifact(
        ...
        type="analysis_data",
        metadata={'model': experiment_name, 'loss_type': 'SA-QGFL'}
    )
...
final_results = {
    ...
    'saqgfl_config': {
        'pwa_alpha': class_alphas,
        'mcp_gamma': class_gammas,
        'difficulty_threshold': args.difficulty_threshold,
        'quality_margin': args.quality_margin,
        'hqs_infection_weight': HQS_INFECTION_WEIGHT if args.use_hqs else None,
        'hqs_species_weight': HQS_SPECIES_WEIGHT if args.use_hqs else None,
    },
    'training': {
        'epochs': args.epochs,
        'batch_size': args.batch_size,
...
results_file = run_dir / 'saqgfl_results.json'
...
print("\n" + "=" * 70)
print("SA-QGFL TRAINING COMPLETE")
print("=" * 70)
print(f"Model: {config.model_name}")
